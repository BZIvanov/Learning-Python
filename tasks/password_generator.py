...
def generate_password():
    ...
    num_digits = random.randint(
        1, password_length - 3
    )  # Ensure at least 1 digit, 1 lowercase letter, and 1 uppercase letter
    num_lower = random.randint(
        1, password_length - num_digits - 2
    )  # Ensure at least 1 lowercase letter and 1 uppercase letter
    num_upper = (
        password_length - num_digits - num_lower - 1
    )  # Remainder for uppercase letters, leaving space for special character(s)
    ...
    symbols = random.choices(
        string.punctuation,
        k=random.randint(1, password_length - num_digits - num_lower - num_upper),
